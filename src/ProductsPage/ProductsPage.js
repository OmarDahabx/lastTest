  ...
  return (
    <div>
      <Container className="mt-5">
        <Row>
        {products.map((p) => (
        <Col md="3" className="my-3 text-center" key={p.id}>
          <img src={p.image} className="img-fluid" style={{height:"300px"}}></img>
          <h5>{p.title}</h5>
          <h6>{p.price}</h6>
...
// function Cart() {
//   const { emptyCart ,isEmpty, totalUniqueItems, items, updateItemQuantity, removeItem, } = useCart();
